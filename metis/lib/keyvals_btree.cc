...
{
    return (btnode_t *)calloc(1, sizeof(btnode_t));
}
...
void pch_kvsbtree::pch_insert_kvs(void *coll, const keyvals_t * k)
{
    btree_t *btree = (btree_t *) coll;
...
int pch_kvsbtree::pch_iter_begin(void *coll, void **iter_)
{
    assert(coll);
    btree_t *btree = (btree_t *) coll;
    if (!btree->nlevel) {
...
int pch_kvsbtree::pch_iter_next_kvs(void *coll, keyvals_t * kvs, void *iter_, int bclear)
{
    iter_t *iter = (iter_t *) iter_;
    if (iter->next == iter->node->nkeys) {
	if (!iter->node->next)
	    return 1;
	iter->node = reinterpret_cast<btnode_t *>(iter->node->next);
	iter->next = 0;
    }
