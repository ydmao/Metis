 ...
 */
static void
reduce_or_group(pc_handler_t * pch, void **elems, int *subsize,
		int lcpu, int ncpus)
{
    const int psz = pch->pch_get_pair_size();
    if (instanceOf<pch_kvsarray>(pch)) {
...
static void
free_arr_colls(void *acolls, int ncolls, pc_handler_t * pch)
{
    const int parrsz = pch->pch_get_parr_size();
    for (int i = 0; i < ncolls; i++) {
...
psrs(void *acolls, int ncolls, int ncpus, int lcpu,
     pc_handler_t * pch, pair_cmp_t pcmp, int doreduce)
{
    const int parrsz = pch->pch_get_parr_size();
    const int psz = pch->pch_get_pair_size();
    if (lcpu == main_lcpu) {
