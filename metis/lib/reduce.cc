...
void
reduce_or_groupkvs(pc_handler_t * pch, void **nodes, int n)
{
...
void
reduce_or_groupkv(pc_handler_t * pch, void **nodes, int n,
		  group_emit_t meth, void *arg)
{
	...
	int end = start + 1;
	while (end < int(total_len) && !keycmp(arr[start].key, arr[end].key))
	    end++;
